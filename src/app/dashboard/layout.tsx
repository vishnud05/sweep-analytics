...
const Sidebar = ({ onClose }: { onClose?: () => void }) => {
  return (
    <div className="space-y-4 md:space-y-6 relative z-20 flex flex-col h-full">
      {/* logo */}
      <p className="hidden sm:flex items-center text-lg/7 font-semibold text-brand-900">
                ...
                {items.map((item, i) => (
                  <Link
                    ...
                    className={cn(
                      ...
                      "w-full justify-start group flex items-center gap-x-2.5 rounded-md px-2 py-1.5 text-sm font-medium leading-6 hover:bg-secondary transition duration-300"
                    )}
                    onClick={onClose}
                  >
                    <item.icon className="size-4 text-zinc-500 group-hover:text-zinc-700 transition" />
                    {item.text}
                  </Link>
                ))}
              </div>
...
const Layout = ({ children }: PropsWithChildren) => {
  ...
  return (
    <div className="relative h-screen flex flex-col md:flex-row  overflow-hidden">
      {/* sidebar for desktop */}
      <div className="hidden md:block w-64 lg:w-80 border-r border-border p-6 h-full text-brand-900 relative z-10">
        <Sidebar />
      </div>
